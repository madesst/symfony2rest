...
abstract class BaseProduct extends BaseObject implements Persistent
{
     ...
     * Peer class name
     */
    const PEER = 'Madesst\\RestBundle\\Propel\\ProductPeer';
    ...
    public function setUpdated($v)
    {
        ...
        if ($this->updated !== null || $dt !== null) {
            ...
            if ($currentDateAsString !== $newDateAsString) {
                ...
                $this->modifiedColumns[] = ProductPeer::UPDATED;
            }
        } // if either are not null
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if (null === $this->id) {
            try {
                $stmt = $con->query("SELECT nextval('product_id_seq')");
                $row = $stmt->fetch(PDO::FETCH_NUM);
                $this->id = $row[0];
            } catch (Exception $e) {
        ...
        if ($this->isColumnModified(ProductPeer::UPDATED)) {
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ProductPeer::UPDATED)) $criteria->add(ProductPeer::UPDATED, $this->updated);
